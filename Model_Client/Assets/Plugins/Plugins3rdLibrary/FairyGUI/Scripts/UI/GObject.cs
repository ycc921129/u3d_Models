...
namespace FairyGUI
{
    public class GObject : EventDispatcher
    {
        #region [Du 修改] 自定义参数
        public int rt_Id = 0;
        public string rt_Info = string.Empty;
        public float rt_Value = 0.0f;
        public object rt_Param = null;
        public FutureCore.BaseVO rt_VO = null;
        public Action rt_Func = null;

        public GTweener rt_GTweener = null;
        public DG.Tweening.Tweener rt_Tweener = null;
        public GObject rt_GObject = null;
        public GComponent rt_GComponent = null;
        public UnityEngine.Object rt_UObject = null;
        #endregion

        #region [Du 修改] 自定义扩展
        public const string Ex_RawURL_PREFIX = "ui://";

        public string ex_rawElementId { get; private set; }
        public string ex_rawElementName { get; private set; }

        public string Ex_GetUrlId()
        {
            return packageItem.owner.id + packageItem.id;
        }
        public string Ex_GetUrl()
        {
            return Ex_RawURL_PREFIX + packageItem.owner.id + packageItem.id;
        }

        public string Ex_GetParentUrlId()
        {
            return parent.packageItem.owner.id + parent.packageItem.id;
        }
        public string Ex_GetParentUrl()
        {
            return Ex_RawURL_PREFIX + parent.packageItem.owner.id + parent.packageItem.id;
        }

        public string Ex_GetMultiLangText()
        {
            return TranslationHelper.GetLangTextByName(Ex_GetParentUrlId(), ex_rawElementName);
        }
        #endregion
