...
namespace FairyGUI
{
    ...
    public class GProgressBar : GComponent
    {
        ...
        /// [Du 修改]
        /// </summary>
        public GTweener Ex_TweenValue(double startValue, double endValue, float duration)
        {
            if (startValue != -1)
            {
                value = startValue;
            }
            return TweenValue(endValue, duration);
        }

        /// <summary>
        /// [Du 修改]
        /// </summary>
        public GTweener Ex_TweenValueLinear(double startValue, double endValue, float duration)
        {
            if (startValue != -1)
            {
                value = startValue;
            }
            return TweenValue(endValue, duration).SetEase(EaseType.Linear).SetIgnoreEngineTimeScale(false);
        }
        #endregion
    }
}