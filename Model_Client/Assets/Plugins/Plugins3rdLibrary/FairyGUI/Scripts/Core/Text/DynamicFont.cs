...
namespace FairyGUI
{
    ...
    public class DynamicFont : BaseFont
    {
        ...
        private float _fontOffset = 1.07f; //系数越大字体越往下
        ...
        public DynamicFont(string name, Font font)
        {
            ...
            // [修改] FGUI字体优化 动态高清字体 添加注释 keepCrisp高清字体模式 动态字体锐化 字号只使用偶数
            this.keepCrisp = true;
            ...
            if (name.ToLower().IndexOf("bold") == -1)
            {
                ...
                this.customBold = IsFguiCustomBoldAndItalic;
                this.customBoldAndItalic = IsFguiCustomBoldAndItalic;
            }
        ...
        void LoadFont()
        {
            ...
            // [修改]
            //Try to load name.ttf in Resources/DynamicFont/
            if (_font == null)
                // [修改]
                _font = (Font)Resources.Load("DynamicFont/" + name, typeof(Font));
        ...
        RenderInfo GetRenderInfo(int size)
        {
            ...
            if (!_renderInfo.TryGetValue(size, out result))
            {
                ...
                result.yIndent = (int)(y0 * _fontOffset);
