...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : TlsContext
    {
        ...
        {
            // ODO[tls13] Ensure early_exporter_master_secret is available suitably early!
            if (!IsConnected)
        ...
        public virtual byte[] ExportKeyingMaterial(string asciiLabel, byte[] context, int length)
        {
            ...
            /*
             * ODO[tls13] Introduce a TlsExporter interface? Avoid calculating (early) exporter
        ...
        protected virtual TlsSecret CheckEarlyExportSecret(TlsSecret secret)
        {
            if (null == secret)
            {
                // ODO[tls13] For symmetry with normal export, ideally available for NotifyHandshakeBeginning() only
