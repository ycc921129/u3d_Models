...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : DatagramTransport
    {
        ...
        {
            // ODO[tls-port] Can we support interrupted IO on .NET?
            //try
            //{
        ...
        // ODO[dtls13] Review the draft/RFC (legacy_record_version) to see if readVersion can be removed
        ...
        internal virtual void InitPendingEpoch(TlsCipher pendingCipher)
        {
            ...
            /*
             * ODO "In order to ensure that any given sequence/epoch pair is unique, implementations
        ...
        public virtual void Send(byte[] buf, int off, int len)
        {
            ...
            if (m_inHandshake || m_writeEpoch == m_retransmitEpoch)
            {
                ...
                if (handshakeType == HandshakeType.finished)
                {
                    ...
                    // ODO Send change_cipher_spec and finished records in single datagram?
        ...
        // ODO Include 'currentTimeMillis' as an argument, use with Timeout, resetHeartbeat
        /// <exception cref="IOException"/>
        private int ProcessRecord(int received, byte[] record, byte[] buf, int off)
        {
            ...
            // ODO[dtls13] Deal with opaque record type for 1.3 AEAD ciphers
            ...
            if (null != m_readVersion && !m_readVersion.Equals(recordVersion))
            {
                 ...
                 * 
                 * ODO Revisit how 'readVersion' works, since this is quite awkward.
                 */
            ...
            case ContentType.alert:
            {
                if (decoded.len == 2)
                {
                    ...
                    // ODO Can close_notify be a fatal alert?
            ...
            case ContentType.application_data:
            {
                if (m_inHandshake)
                {
                    // ODO Consider buffering application data for new epoch that arrives
            ...
            case ContentType.handshake:
            {
                if (!m_inHandshake)
                {
                    ...
                    // ODO Consider support for HelloRequest
                    return -1;
                }
                break;
            }
