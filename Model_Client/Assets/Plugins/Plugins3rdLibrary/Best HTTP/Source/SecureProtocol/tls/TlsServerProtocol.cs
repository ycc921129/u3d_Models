...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : TlsProtocol
    {
        ...
        {
            // ODO[tls13] In future there might be other reasons for a HelloRetryRequest.
            if (m_retryGroup < 0)
            ...
            HandshakeMessageInput clientHelloMessage, bool afterHelloRetryRequest)
        {
            ...
            if (afterHelloRetryRequest)
            {
                ...
                {
                    // ODO[tls13] Maybe filter the offered PSKs by PRF algorithm before server selection instead
                    if (selectedPsk.m_psk.PrfAlgorithm != securityParameters.PrfAlgorithm)
                        throw new TlsFatalAlert(AlertDescription.illegal_parameter);
                }

                /*
                 * ODO[tls13] Confirm fields in the ClientHello haven't changed
                 * 
            ...
            else
            {
                ...
                {
                    // ODO[tls13] Constrain selection when PSK selected
                ...
                {
                    /*
                     * ODO[tls13] RFC 8446 4.2.7. As of TLS 1.3, servers are permitted to send the
            ...
            HandshakeMessageInput clientHelloMessage)
        {
            ...
            // ODO[tls13] Negotiate cipher suite first?
            ...
            if (m_resumedSession)
            {
                ...
                {
                    /*
                     * DO[resumption] ProvTlsServer currently only resumes EMS sessions. Revisit this
        ...
        protected virtual void Handle13HandshakeMessage(short type, HandshakeMessageInput buf)
        {
            ...
            {
                /*
                 * ODO[tls13] Abbreviated handshakes (PSK resumption)
                 * 
        ...
        protected override void HandleHandshakeMessage(short type, HandshakeMessageInput buf)
        {
            ...
            case HandshakeType.client_hello:
            {
                ...
                case CS_START:
                {
                    ...
                    // Server certificate
                    {
                        ...
                        // ODO[RFC 3546] Check whether empty certificates is possible, allowed, or excludes
                        // CertificateStatus
            ...
            case HandshakeType.finished:
            {
                ...
                case CS_CLIENT_CERTIFICATE_VERIFY:
                {
                    ...
                    {
                        /*
                         * ODO[new_session_ticket] Check the server-side rules regarding the session ID, since
        ...
        {
            // ODO[tls13] This currently just duplicates 'receiveCertificateMessage'
        ...
        protected void Receive13ClientCertificateVerify(MemoryStream buf)
        {
            ...
            // DO[tls13] Actual structure is 'CertificateVerify' in RFC 8446, consider adding for clarity
        ...
        {
            // ODO[tls13] Avoid extra copy; use placeholder to write opaque-16 data directly to message buffer
        ...
        protected virtual void Send13ServerHelloCoda(ServerHello serverHello, bool afterHelloRetryRequest)
        {
            ...
            else
            {
                ...
                {
                    /*
                     * ODO[tls13] Note that we are expecting the TlsServer implementation to take care of
        ...
        protected virtual void SendCertificateStatusMessage(CertificateStatus certificateStatus)
        {
            ...
            // ODO[tls13] Ensure this cannot happen for (D)TLS1.3+
