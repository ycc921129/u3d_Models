...
namespace BestHTTP.SecureProtocol.Org.BouncyCastle.Tls
{
        ...
        : TlsProtocol
    {
        ...
        protected virtual void Handle13HandshakeMessage(short type, HandshakeMessageInput buf)
        {
            ...
            case HandshakeType.certificate_request:
            {
                ...
                case CS_END:
                {
                    // ODO[tls13] Permit post-handshake authentication if we sent post_handshake_auth extension
                    throw new TlsFatalAlert(AlertDescription.unexpected_message);
                }
                case CS_SERVER_ENCRYPTED_EXTENSIONS:
                {
            ...
            case HandshakeType.finished:
            {
                ...
                case CS_SERVER_CERTIFICATE_VERIFY:
                {
                    ...
                    /*
                     * ODO[tls13] After receiving the server's Finished message, if the server has accepted early
        ...
        protected override void HandleHandshakeMessage(short type, HandshakeMessageInput buf)
        {
            ...
            case HandshakeType.server_hello:
            {
                ...
                case CS_CLIENT_HELLO:
                {
                    ...
                    // ODO[tls13] Only treat as HRR if it's TLS 1.3??
                    if (serverHello.IsHelloRetryRequest())
                    {
            ...
            case HandshakeType.certificate_request:
            {
                ...
                case CS_SERVER_KEY_EXCHANGE:
                {
                    ...
                    /*
                     * ODO Give the client a chance to immediately select the CertificateVerify hash
        ...
        protected virtual void Process13ServerHello(ServerHello serverHello, bool afterHelloRetryRequest)
        {
            ...
            /*
             * ODO[tls13] RFC 8446 4.4.2.1. OCSP Status and SCT Extensions.
             * 
        ...
        protected virtual void Process13ServerHelloCoda(ServerHello serverHello, bool afterHelloRetryRequest)
        {
            ...
            if (!afterHelloRetryRequest)
            {
                ...
                /*
                 * ODO[tls13] If offering early_data, the record is placed immediately after the first
                 * ClientHello.
                 */
                /*
                 * ODO[tls13] Ideally wait until just after Server Finished received, but then we'd need to defer
        ...
        protected virtual void ProcessServerHello(ServerHello serverHello)
        {
            ...
            if (m_serverExtensions != null)
            {
                foreach (int extType in m_serverExtensions.Keys)
                {
                    ...
                    if (m_resumedSession)
                    {
                        // ODO[compat-gnutls] GnuTLS test server sends server extensions e.g. ec_point_formats
                        // ODO[compat-openssl] OpenSSL test server sends server extensions e.g. ec_point_formats
                        // ODO[compat-polarssl] PolarSSL test server sends server extensions e.g. ec_point_formats
    //                    throw new TlsFatalAlert(AlertDescription.illegal_parameter);
                    }
                }
            }
            ...
            // ODO[compat-gnutls] GnuTLS test server fails to send renegotiation_info extension when resuming
            ...
            if (sessionServerExtensions != null && sessionServerExtensions.Count > 0)
            {
                ...
                /*
                 * ODO It's surprising that there's no provision to allow a 'fresh' CertificateStatus to be sent in
                 * a session resumption handshake.
                 */
                if (!m_resumedSession)
                {
                    // ODO[tls13] See RFC 8446 4.4.2.1
        ...
        {
            // ODO[tls13] Support for post_handshake_auth
        ...
        protected virtual void Receive13NewSessionTicket(MemoryStream buf)
        {
            ...
            // ODO[tls13] Do something more than just ignore them
    ...
    //        } NewSessionTicket;
        ...
        protected virtual void Receive13ServerCertificateVerify(MemoryStream buf)
        {
            ...
            // ODO[tls13] Actual structure is 'CertificateVerify' in RFC 8446, consider adding for clarity
        ...
        protected virtual void Send13ClientHelloRetry()
        {
            ...
            /*
             * ODO[tls13] Optionally adding, removing, or changing the length of the "padding"
             * extension [RFC7685].
             */
        ...
        protected virtual void SendClientHello()
        {
            ...
            else
            {
                ...
                {
                    // ODO[tls13] Prevent offering SSLv3 AND TLSv13?
            ...
            /*
             * ODO RFC 5077 3.4. When presenting a ticket, the client MAY generate and include a
            ...
            // DO[tls13-psk] Perhaps don't add key_share if external PSK(s) offered and 'psk_dhe_ke' not offered  
            ...
            else
            {
                ...
                {
                    // ODO[tls13] Probably want to not add this if no pre-TLSv13 versions offered?
