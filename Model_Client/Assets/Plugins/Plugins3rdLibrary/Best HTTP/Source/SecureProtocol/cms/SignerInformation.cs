	...
	*/
	public class SignerInformation
	{
		...
		*/
		...
		{
			// ODO There are several checks implied by the RFC3852 comments that are missing
		...
		*/
			...
			AsymmetricKeyParameter	key)
		{
			...
			if (sigAlgOid.Equals(Asn1.Pkcs.PkcsObjectIdentifiers.IdRsassaPss))
			{
				...
				try
				{
					// ODO Provide abstract configuration mechanism
			...
			{
				// ODO Probably too strong a check at the moment
				//				if (sigParams != null)
			...
			try
			{
				...
				else
				{
					...
					{
						// ODO Get rid of this exception and just treat content==null as empty not missing?
			...
			try
			{
				...
				if (signedAttributeSet == null)
				{
					...
					else if (content != null)
					{
                        try
                        {
                            // ODO Use raw signature of the hash value instead
		...
		*/
		...
		{
			// ODO Perform checks from RFC 3852 11.4
