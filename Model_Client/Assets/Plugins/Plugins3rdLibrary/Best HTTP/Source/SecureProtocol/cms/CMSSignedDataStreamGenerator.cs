...
//						Asn1.Cms.AttributeTable unsigned = _unsAttr.getAttributes(Collections.unmodifiableMap(parameters));
					...
					// ODO[RSAPSS] Need the ability to specify non-default parameters
		...
		*/
            ...
            : BaseOutputStream
        {
            ...
            private void DoClose()
            {
                ...
                // ODO Parent context(s) should really be be closed explicitly
				...
				// ODO If the digest OIDs for precalculated signers weren't mixed in with
                ...
                //
				{
					...
					{
						// ODO Verify the content type and calculated digest match the precalculated SignerInfo
//						if (!signer.ContentType.Equals(_contentOID))
//						{
//							// ODO The precalculated content type did not match - error?
//						}
//
//						byte[] calculatedDigest = (byte[])outer._digests[signer.DigestAlgOid];
//						if (calculatedDigest == null)
//						{
//							// ODO We can't confirm this digest because we didn't calculate it - error?
//						}
//						else
//						{
//							if (!Arrays.AreEqual(signer.GetContentDigest(), calculatedDigest))
//							{
//								// ODO The precalculated digest did not match - error?
//							}
//						}
