        ...
        */
			...
			Asn1Set	certSet)
		{
			...
			foreach (Asn1Encodable ae in certSet)
			{
				try
				{
					...
					{
						// ODO Build certificate directly from sequence?
			...
			Asn1Set	crlSet)
		{
			...
			foreach (Asn1Encodable ae in crlSet)
			{
				try
				{
					// ODO Build CRL directly from ae.ToAsn1Object()?
            ...
            string digestOID)
        {
            ...
            else if (key is ECPrivateKeyParameters)
            {
                ...
                {
                    // ODO Should we insist on algName being one of "EC" or "ECDSA", as Java does?
