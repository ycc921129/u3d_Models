...
namespace Pomelo.Client
{
    public class MessageProtocol
    {
        ...
        public MessageProtocol(JObject dict, JObject serverProtos, JObject clientProtos)
        {
            ...
            JEnumerable<JToken> keys = dict.Children();
        ...
        public byte[] encode(string route, uint id, JObject msg)
        {
            ...
            //Encode body
            byte[] body;
            if (encodeProtos.Property(route) != null)
            {
        ...
        public Message decode(byte[] buffer)
        {
            ...
            JObject msg;   
            if (decodeProtos.Property(route) != null)
            {
                msg = protobuf.decode(route, body);
            }  
            else
            {
                msg = SerializeUtil.ToObject<JObject>(Encoding.UTF8.GetString(body));  
            }       
