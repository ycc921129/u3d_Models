...
namespace FutureCore
{
    public class EventNode : MonoBehaviour
    {
        ...
        public delegate void EventListenerDele(EventNodeData data);
            ...
            bool _dispatchOnce = false)
        {
            ...
            {
                LogUtil.Log("LogicNode, AttachListenerNow: " + _listener + " is already in list for event: " +
                          type);
                return;
            }
        ...
        private void DetachListenerNow(string type, EventListenerDele _listener)
        {
            ...
            {
                LogUtil.LogError("回调函数或者 事件类型不能为空");
                return;
            }
        ...
        {
            QueueEvent(new EventNodeData(key));
        }
        ...
        {
            var data = new EventNodeData<T>(key, value);
            QueueEvent(data);
        }
        ...
        {
            QueueEventNow(new EventNodeData(key.ToString()));
        }
        ...
        {
            QueueEventNow(new EventNodeData(key));
        }
        ...
        {
            QueueEventNow(new EventNodeData<T>(key, value));
        }
        ...
        public void QueueEventNow(EventNodeData data)
        {
            if (threadSafe)
                lock (evtQueueLock)
                {
        ...
        private void _DispatchEventLock()
        {
            ...
            lock (evtQueueLock)
            {
                EventNodeData evt = null;
                //发布自己的event
                var countEventQueue = m_eventQueueNow.Count;
                while (countEventQueue > 0)
                {
                    evt = m_eventQueueNow.Dequeue() as EventNodeData;
                    countEventQueue--;
        ...
        /// <returns></returns>
        private void TriggerEvent(EventNodeData data)
        {
    ...
    public class EventNodeListener
    {
        private readonly List<EventListenerData> mlistener = new List<EventListenerData>();

        public void AddListener(EventNode dis, EventNode.EventListenerDele dele, int type
            , int _priority = 0, bool _dispatchOnce = false)
        {
            AddListener(dis, dele, type.ToString(), _priority, _dispatchOnce);
        }

        public void AddListener(EventNode dis, EventNode.EventListenerDele dele, string type
            , int _priority = 0, bool _dispatchOnce = false)
        {
            ...
            {
                LogUtil.LogWarning("添加消息失败重复添加消息id=" + type);
            }
        }

        public void DetachListener(EventNode dis, EventNode.EventListenerDele dele, string type)
        {
            if (dis.HasListener(type, dele))
            {
        ...
        {
            public EventNode.EventListenerDele dele;
            public EventNode dis;
            public bool dispatchOnce;
            public int priority;
            public string type;

            public EventListenerData(EventNode _dis,
                EventNode.EventListenerDele _dele, string _type,
                int _priority = 0, bool _dispatchOnce = false)
            {
                dis = _dis;
                dele = _dele;
