...
{
    public static class ExpressionParserTest
    {
        public static void Test()
        {
            var parser = new ExpressionParser();
            Expression exp = parser.EvaluateExpression("(5+3)*8^2-5*(-2)");
            UnityEngine.Debug.Log("Result: " + exp.Value);  // prints: "Result: 522"

            UnityEngine.Debug.Log("Result: " + parser.Evaluate("ln(e^5)"));  // prints: "Result: 5"
            ...
            UnityEngine.Debug.Log("sin(45°): " + exp2.Value); // prints "sin(45°): 0.707106781186547" 


            // convert the expression into a delegate:

            var sinFunc = exp2.ToDelegate("x");
            UnityEngine.Debug.Log("sin(90°): " + sinFunc(90)); // prints "sin(90°): 1" 
            ...
            parser.AddFunc("test", (p) => {
                UnityEngine.Debug.Log("TEST: " + p.Length);
                return 42;
            });
            UnityEngine.Debug.Log("Result: " + parser.Evaluate("2*test(1,5)")); // prints "TEST: 2" and "Result: 84"


            // NOTE: functions without parameters are not supported, use "constants" instead:
            parser.AddConst("meaningOfLife", () => 42);
            UnityEngine.Debug.Log("Result: " + parser.Evaluate("2*meaningOfLife")); // prints "Result: 84"
        }
    }
