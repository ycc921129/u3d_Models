...
{
    internal sealed partial class DebuggerConsoleManager : ManagerModule, IDebuggerConsoleManager
    {
        /// <summary>
        /// 调试器窗口组。
        /// </summary>
        private sealed class DebuggerConsoleWindowGroup : IDebuggerConsoleWindowGroup
        {
            private readonly List<KeyValuePair<string, IDebuggerConsoleWindow>> m_DebuggerWindows;
            private int m_SelectedIndex;
            private string[] m_DebuggerWindowNames;

            public DebuggerConsoleWindowGroup()
            {
                m_DebuggerWindows = new List<KeyValuePair<string, IDebuggerConsoleWindow>>();
                m_SelectedIndex = 0;
            ...
            /// 获取当前选中的调试器窗口。
            /// </summary>
            public IDebuggerConsoleWindow SelectedWindow
            {
                get
                {
            ...
            {
                foreach (KeyValuePair<string, IDebuggerConsoleWindow> debuggerWindow in m_DebuggerWindows)
                {
            ...
            public IDebuggerConsoleWindow GetDebuggerWindow(string path)
            {
                ...
                DebuggerConsoleWindowGroup debuggerWindowGroup = (DebuggerConsoleWindowGroup)InternalGetDebuggerWindow(debuggerWindowGroupName);
            ...
            public void RegisterDebuggerWindow(string path, IDebuggerConsoleWindow debuggerWindow)
            {
                if (string.IsNullOrEmpty(path))
                {
                ...
                if (pos < 0 || pos >= path.Length - 1)
                {
                    ...
                    m_DebuggerWindows.Add(new KeyValuePair<string, IDebuggerConsoleWindow>(path, debuggerWindow));
                ...
                else
                {
                    ...
                    if (debuggerWindowGroup == null)
                    {
                        ...
                        debuggerWindowGroup = new DebuggerConsoleWindowGroup();
                        m_DebuggerWindows.Add(new KeyValuePair<string, IDebuggerConsoleWindow>(debuggerWindowGroupName, debuggerWindowGroup));
                        RefreshDebuggerWindowNames();
                    }
            ...
            public bool UnregisterDebuggerWindow(string path)
            {
                ...
                if (pos < 0 || pos >= path.Length - 1)
                {
                    IDebuggerConsoleWindow debuggerWindow = InternalGetDebuggerWindow(path);
                    bool result = m_DebuggerWindows.Remove(new KeyValuePair<string, IDebuggerConsoleWindow>(path, debuggerWindow));
            ...
            private IDebuggerConsoleWindow InternalGetDebuggerWindow(string name)
            {
                foreach (KeyValuePair<string, IDebuggerConsoleWindow> debuggerWindow in m_DebuggerWindows)
                {
