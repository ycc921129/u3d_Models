...
namespace FutureCore.FC_Debuger
{
    ...
    {
        [Serializable]
        private sealed class ConsoleWindow : IDebuggerConsoleWindow
        {
            ...
            public void OnDraw()
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    m_InfoFilter = GUILayout.Toggle(m_InfoFilter, TextSbUtil.Format("Info ({0})", m_InfoCount.ToString()), GUILayout.Width(90f));
                    m_WarningFilter = GUILayout.Toggle(m_WarningFilter, TextSbUtil.Format("Warning ({0})", m_WarningCount.ToString()), GUILayout.Width(90f));
                    m_ErrorFilter = GUILayout.Toggle(m_ErrorFilter, TextSbUtil.Format("Error ({0})", m_ErrorCount.ToString()), GUILayout.Width(90f));
                    m_FatalFilter = GUILayout.Toggle(m_FatalFilter, TextSbUtil.Format("Fatal ({0})", m_FatalCount.ToString()), GUILayout.Width(90f));
                }
                ...
                GUILayout.BeginVertical("box");
                {
                    m_StackScrollPosition = GUILayout.BeginScrollView(m_StackScrollPosition, GUILayout.Height(100f));
                    {
                        if (m_SelectedNode != null)
                        {
                            ...
                            GUILayout.Label(TextSbUtil.Format("<color=#{0}{1}{2}{3}><b>{4}</b></color>", color.r.ToString("x2"), color.g.ToString("x2"), color.b.ToString("x2"), color.a.ToString("x2"), m_SelectedNode.LogMessage));
                            if (GUILayout.Button("COPY", GUILayout.Width(60f), GUILayout.Height(30f)))
                            {
                                m_TextEditor.text = TextSbUtil.Format("{0}{2}{2}{1}", m_SelectedNode.LogMessage, m_SelectedNode.StackTrack, Environment.NewLine);
            ...
            public void GetRecentLogs(List<LogNode> results)
            {
                if (results == null)
                {
                    LogUtil.LogError("Results is invalid.");
                    return;
                }
            ...
            public void GetRecentLogs(List<LogNode> results, int count)
            {
                ...
                if (count <= 0)
                {
                    LogUtil.LogError("Count is invalid.");
                    return;
                }
            ...
            private void OnLogMessageReceived(string logMessage, string stackTrace, LogType logType)
            {
                ...
                {
                    Debugger.logNodeObjectPool.Release(m_LogNodes.Dequeue());
                }
            }
            ...
            private string GetLogString(LogNode logNode)
            {
                ...
                return TextSbUtil.Format("<color=#{0}{1}{2}{3}>[{4}][{5}] {6}</color>",
