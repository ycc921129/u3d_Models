...
namespace FutureCore
{
    ...
    public class DynamicAssetLoader
    {
        ...
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                T cacheAsset = GetInDynamicCache<T>(assetPath);
                func(cacheAsset);
                return;
            }
        ...
        private void LoadObject(string assetPath, LoadedObjectFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                UObject cacheAsset = GetInDynamicCache<UObject>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadPrefab(string assetPath, LoadedGameObjectFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                GameObject cacheAsset = GetInDynamicCache<GameObject>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadSprite(string assetPath, LoadedSpriteFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                Sprite cacheAsset = GetInDynamicCache<Sprite>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadTexture(string assetPath, LoadedTextureFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                Texture cacheAsset = GetInDynamicCache<Texture>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadAudioClip(string assetPath, LoadedAudioClipFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                AudioClip cacheAsset = GetInDynamicCache<AudioClip>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadTextAsset(string assetPath, LoadedTextAssetFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                TextAsset cacheAsset = GetInDynamicCache<TextAsset>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadMaterial(string assetPath, LoadedMaterialFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                Material cacheAsset = GetInDynamicCache<Material>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadShader(string assetPath, LoadedShaderFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                Shader cacheAsset = GetInDynamicCache<Shader>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadFont(string assetPath, LoadedFontFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                Font cacheAsset = GetInDynamicCache<Font>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadVideoClip(string assetPath, LoadedVideoClipFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                VideoClip cacheAsset = GetInDynamicCache<VideoClip>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadAnimatorController(string assetPath, LoadedRuntimeAnimatorControllerFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                RuntimeAnimatorController cacheAsset = GetInDynamicCache<RuntimeAnimatorController>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        public void LoadScriptableObject(string assetPath, LoadedScriptableObjectFunc func, bool isSync = IsSyncMode)
        {
            if (mgr.HasDynamicCache(assetPath))
            {
                ScriptableObject cacheAsset = GetInDynamicCache<ScriptableObject>(assetPath);
                func(cacheAsset);
                return;
            }
            if (mgr.HasCache(assetPath))
        ...
        private T SyncLoad<T>(string assetPath) where T : UObject
        {
            ...
            AddLoadedPath(assetPath);
            AddDynamicAssetCache(assetPath, asset);
            return asset;
        }
        ...
        {
            return mgr.GetInDynamicCache<T>(assetPath);
        }
        #endregion

        #region Func
        private void AddLoadedPath(string assetPath)
        {
            loadedPaths.Add(assetPath);
        }

        private void AddDynamicAssetCache(string assetPath, UObject asset)
        {
            mgr.AddDynamicCache(assetPath, asset);
        }
        ...
        public void Release()
        {
            if (!IsResourcesMode)
            {
                for (int i = 0; i < loadedPaths.Count; i++)
                {
                    string assetPath = loadedPaths[i];
                    mgr.Unload(assetPath);

                    mgr.DelDynamicCache(assetPath);
                }
            }
