...
{
    /// <summary>
    /// 如果是运行时进行的合并，一定要调用Mesh.UploadMeshData(true)
    /// 表示将mesh在.NET托管堆中的byte[]清空，不然内存就双份了
    /// </summary>
    public class CombineSubMesh : MonoBehaviour
    {
        [HideInInspector]
        ...
        public static void Combine(GameObject gameObject)
        {
            if (gameObject)
            {
                ...
                var go = new GameObject("CombineSubMesh", typeof(MeshFilter), typeof(MeshRenderer));
                go.transform.position = centerPos;
                go.AddComponent<CombineSubMesh>().lightmapIndex = lightmap;

                var mesh = new Mesh();
                mesh.CombineMeshes(combine, true, true, true);
                //合拼会自动生成UV3，但是我们并不需要，可以这样删除
                mesh.uv3 = null;
                AssetDatabase.CreateAsset(mesh, "Assets/CombineSubMesh.asset");
                go.GetComponent<MeshFilter>().sharedMesh = mesh;
                go.GetComponent<MeshRenderer>().sharedMaterial = material;
                if (go)
                {
                    PrefabUtility.SaveAsPrefabAssetAndConnect(go, Application.dataPath + "/CombineSubMesh.prefab", InteractionMode.AutomatedAction);
                }
            }
