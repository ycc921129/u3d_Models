		...
		void DrawAssetUsageAncestry(Rect position, float availableWidth, BuildInfo buildReportToDisplay, AssetDependencies assetDependencies)
		{
			...
			for (int n = 0, len = _assetUsageAncestry.Count; n < len; ++n)
			{
				...
				else if (BuildReportTool.Options.IsAssetUsageLabelTypeOnStandard)
				{
					if (n < len - 1)
					{
						// odo determine if _assetUsageAncestry[n] has extra info and add that to width
				...
				else if (BuildReportTool.Options.IsAssetUsageLabelTypeOnStandard)
				{
					...
					if (n < len - 1)
					{
						// odo determine if _assetUsageAncestry[n] has extra info and draw that and different kind of arrow
		...
		int GetNumberOfAssetUsageAncestryRows(List<AssetUsageAncestry> list, float availableWidth)
		{
			...
			for (int n = 0, len = list.Count; n < len; ++n)
			{
				...
				else if (BuildReportTool.Options.IsAssetUsageLabelTypeOnStandard)
				{
					if (n < len - 1)
					{
						// odo determine if list[n] has extra info and add that to width
						widthToAdd = assetUsageArrow.width + assetUsageArrowStyle.padding.horizontal;
					}
				}
				else if (BuildReportTool.Options.IsAssetUsageLabelTypeOnMinimal)
