    ...
    abstract public class AbstractMemoryReader
    {
        ...
        public string ReadFieldValueAsString(System.UInt64 address, PackedManagedType type)
        {
            ...
            if (type.managedTypesArrayIndex == m_Snapshot.coreTypes.systemString)
            {
                ...
                // ODO: HACK: Reading a static pointer, points actually into the HEAP. However,
            ...
            if (type.isValueType)
            {
                ...
                try
                {
                    ...
                    for (var n = 0; n < count; ++n)
                    {
                        var offset = 0;
                        if (n > 0)
                            offset += instanceFields[n].offset - m_Snapshot.virtualMachineInformation.objectHeaderSize; // ODO: this is trial&error. make sure to understand it!
