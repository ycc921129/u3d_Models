...
public class ReferenceFinderData
{
    ...
    private void WriteToChache()
    {
        ...
        using (FileStream fs = File.OpenWrite(CACHE_PATH))
        {
            ...
            {
                if (assetDict[guid] != null)
                {
                    if (assetDict[guid].dependencies != null)
                    {
                        List<string> dependenciesList = assetDict[guid].dependencies;
                        int[] indexes = dependenciesList.Select(s =>
                        {
