...
{
	public class UIAssetsCheckWindow : EditorWindow
	{
		#region 定义
		public class TextureDetails : IEquatable<TextureDetails>
		{
			...
			public bool isgui;
			public HashSet<Object> FoundObjects = new HashSet<Object>();
		...
		private string[] inspectToolbarStrings = { "纹理", "材质球" };
		private string[] inspectToolbarStrings2 = { "纹理", "材质球", "引用丢失" };
		...
		private bool OnlyZeroTexture = false;
		...
		[MenuItem("[FC Toolkit]/UI/UI资源检查窗口", false, 0)]
		private static void Init()
		{
			UIAssetsCheckWindow window = GetWindow<UIAssetsCheckWindow>("UI资源检查窗口");
		...
		private void OnGUI()
		{
			...
			OnlyZeroTexture = GUILayout.Toggle(OnlyZeroTexture, "仅显示零引用", GUILayout.Width(300));
			GUI.color = defColor;
			GUILayout.BeginArea(new Rect(position.width - 125, 5, 200, 100));

			//if (GUILayout.Button("检查资源", GUILayout.Width(100), GUILayout.Height(25)))
			//    CheckResources();
			//GUILayout.Space(5);
			//if (GUILayout.Button("清除缓存", GUILayout.Width(100), GUILayout.Height(25)))
			//    Resources.UnloadUnusedAssets();

			if (GUILayout.Button("检查UI资源", GUILayout.Width(100), GUILayout.Height(25)))
				CheckAtlasResources();
		...
		private void ListTextures()
		{
			...
			foreach (TextureDetails tDetails in ActiveTextures)
			{
				...
				if (OnlyZeroTexture && tDetails.FoundObjects != null && tDetails.FoundObjects.Count != 0)
					continue;
				GUILayout.BeginHorizontal();
				...
				if (GUILayout.Button(tDetails.FoundObjects.Count + " GO", GUILayout.Width(50)))
				{
					SelectObjects(new List<Object>(tDetails.FoundObjects), ctrlPressed);
				}
				if (tDetails.FoundObjects.Count == 0)
				{
					if (GUILayout.Button("删除", GUILayout.Width(50)))
					{
						AssetDatabase.DeleteAsset(AssetDatabase.GetAssetPath(tDetails.texture));
						ActiveTextures.Remove(tDetails);
					}
				}
				GUILayout.Label(AssetDatabase.GetAssetPath(tDetails.texture).Replace("Assets/", ""));
				GUILayout.EndHorizontal();
			}
		...
		private void CheckAtlasResources()
		{
			ActiveTextures.Clear();
			ActiveMaterials.Clear();
			ActiveMeshDetails.Clear();
			...
			//获取选中文件夹
			UnityEngine.Object[] arr = Selection.GetFiltered(typeof(UnityEngine.Object), SelectionMode.ExcludePrefab);
			List<string> SelectPaths = new List<string>();
			foreach (var item in arr)
			{
				SelectPaths.Add(AssetDatabase.GetAssetPath(item));
			}
			//获取文件夹下的所有图片
			foreach (var item in SelectPaths)
			{
				foreach (var subitem in Directory.GetFiles(item))
				{
					if (subitem.EndsWith(".meta"))
						continue;
					if (!subitem.StartsWith("Assets/_Game/ResourcesArt/Atlas"))
						continue;
					Texture t = AssetDatabase.LoadAssetAtPath(subitem, typeof(Texture)) as Texture;
					TextureDetails detail = GetTextureDetail(t);
					ActiveTextures.Add(detail);
				}
			...
			string[] ids = AssetDatabase.FindAssets("t:Prefab", new string[] { "Assets/_Game/Resources/Module" });
			...
			foreach (var item in ids)
			{
				...
				if (go && go.GetComponentsInChildren<Renderer>(true) != null)
					renderers.AddRange(go.GetComponentsInChildren<Renderer>(true));
				if (go && go.GetComponentsInChildren<Graphic>(true) != null)
					graphics.AddRange(go.GetComponentsInChildren<Graphic>(true));
				if (go && go.GetComponentsInChildren<MeshFilter>(true) != null)
					meshFilters.AddRange(go.GetComponentsInChildren<MeshFilter>(true));
			}
			...
			foreach (TextureDetails tTextureDetails in ActiveTextures)
			{
				...
				foreach (Renderer renderer in tTextureDetails.FoundInRenderers) FoundObjects.Add(renderer.gameObject);
				foreach (Animator animator in tTextureDetails.FoundInAnimators) FoundObjects.Add(animator.gameObject);
				foreach (Graphic graphic in tTextureDetails.FoundInGraphics) FoundObjects.Add(graphic.gameObject);
				foreach (Button button in tTextureDetails.FoundInButtons) FoundObjects.Add(button.gameObject);
				foreach (MonoBehaviour script in tTextureDetails.FoundInScripts) FoundObjects.Add(script.gameObject);
				tTextureDetails.FoundObjects = FoundObjects;
			}
			...
			{
				if (details1.FoundObjects.Count != details2.FoundObjects.Count)
				{
					return details1.FoundObjects.Count - details2.FoundObjects.Count;
				}
				else
