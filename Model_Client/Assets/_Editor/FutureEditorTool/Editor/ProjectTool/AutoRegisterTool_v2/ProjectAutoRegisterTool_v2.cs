...
//ReplaceUICtrl
        ...
        #region 消息协议
        [MenuItem("[FC Project]/AutoRegister/自动创建与注册消息协议", false, -1)]
        private static void AutoRegisterProtoMsg()
        {
            var detectFolders = new List<string>
        {
            "/_AppBase/ProjectAppBase/_AutoCreator/WSNetProtocolCommon/",
            "/_App/AutoCreator/AutoRegister/WSNetProtocol/",
            "/_AppMM/Protocol/",
        };
            // 1、根据*_data生成消息
            GenMsgCs(detectFolders);
            // 2、根据消息生成WSNetMsg_AutoCreator.cs
            GenWSNetMsg_AutoCreator(detectFolders);
            // 3、根据消息外包装生成 WSNetMgrRegister_AutoCreator.cs
            GenWSNetMgrRegister_AutoCreator(detectFolders);

            AssetDatabase.Refresh();
        }

        private static void GenMsgCs(List<string> detectFolders)
        {
            var genSet = new HashSet<string>();
            foreach (string detectFolder in detectFolders)
            {
                // 查找下面的C2SMsg文件夹和S2CMsg文件夹
                var dataFolders = new List<string> { detectFolder + "C2SData/", detectFolder + "S2CData/" };
                foreach (var dataFolder in dataFolders)
                {
                    string path = Application.dataPath + dataFolder;
                    ...
                    foreach (var fileInfo in pathInfo.GetFiles())
                    {
                        var fileName = fileInfo.Name;
                        if (!fileName.EndsWith("_data.cs"))
                        {
                            continue;
                        }
                        if (!fileName.StartsWith("C2S_") && !fileName.StartsWith("S2C_"))
                        {
                            continue;
                        }
                        string msgName = fileName.Substring(0, fileName.Length - "_data.cs".Length);
                        if (genSet.Contains(msgName))
                            continue;
                        genSet.Add(msgName);
                        string content = "";
                        string targetPath = "";
                        string targetDir = "";
                        if (fileName.StartsWith("C2S_"))
                        {
                            targetDir = string.Format("{0}C2SMsg/", detectFolder);
                            targetPath = string.Format("{0}C2SMsg/{1}.cs", detectFolder, msgName);
                            content = string.Format(
@"using FutureCore;

namespace ProjectApp.Protocol
{{
    public class {0} : C2SJsonProto<{0}_data>
    {{
        public {0}()
        {{
            type = WSNetMsg.{0};
            data = new {0}_data();
                        ...
                        else if (fileName.StartsWith("S2C_"))
                        {
                            targetDir = string.Format("{0}S2CMsg/", detectFolder);
                            targetPath = string.Format("{0}S2CMsg/{1}.cs", detectFolder, msgName);
                            content = string.Format(@"
using FutureCore;

namespace ProjectApp.Protocol
{{
    public class {0} : S2CJsonProto<{0}_data>
    {{
        public {0}()
        {{
            type = WSNetMsg.{0};
        ...
        private static void GenWSNetMsg_AutoCreator(List<string> detectFolders)
        {
            ...
            string lines =
                ...
                "namespace ProjectApp\r\n" +
                "{\r\n" +
                "    public static class WSNetMsg\r\n" +
                "    {\r\n";
            foreach (string detectFolder in detectFolders)
            {
                // 查找下面的C2SMsg文件夹和S2CMsg文件夹
                var dataFolders = new List<string> { detectFolder + "C2SMsg/", detectFolder + "S2CMsg/" };
                foreach (var dataFolder in dataFolders)
                {
                    string path = Application.dataPath + dataFolder;
                    DirectoryInfo pathInfo = new DirectoryInfo(path);
                    ...
                    if (pathInfo.GetFiles().Length > 1)
                    {
                        lines += string.Format("        // gen msg const from {0}\r\n", dataFolder);
                    }
                    foreach (FileInfo file in pathInfo.GetFiles())
                    {
                        ...
                        string msgName = fileName.Substring(0, fileName.Length - ".cs".Length);
                        if (genSet.Contains(msgName))
                            continue;
                        genSet.Add(msgName);

                        lines += string.Format("        public const string {0} = \"{0}\";\r\n", msgName);
                    }
                }
            }
            lines += "    }\r\n";
            ...
            string genPath = Application.dataPath + "/_App/AutoCreator/AutoRegister/WSNetMgr/WSNetMsg_AutoCreator.cs";
            WriteFile(genPath, lines);
            Debug.Log("[ProjectAutoRegisterTool]处理WSNetMsg完成:" + genPath);
        }

        private static void GenWSNetMgrRegister_AutoCreator(List<string> detectFolders)
        {
            var genSet = new HashSet<string>();
            string content =
                AutoCreatorComments +
            ...
            foreach (var detectFolder in detectFolders)
            {
                var msgFolders = new List<string> { "C2SMsg", "S2CMsg" };
                foreach (var msgFolder in msgFolders)
                {
                    var pathStr = detectFolder + msgFolder;
                    string lines = "";
                    string path = Application.dataPath + pathStr;
                    DirectoryInfo pathInfo = new DirectoryInfo(path);
                    if (!pathInfo.Exists)
                    {
                        continue;
                    ...
                    if (pathInfo.GetFiles().Length > 1)
                    {
                        lines += string.Format("            // gen msg Register from {0}\r\n", pathStr);
                    }
                    foreach (FileInfo file in pathInfo.GetFiles())
                    {
                        if (!file.Name.EndsWith(".cs"))
                            continue;
                        if (!file.Name.StartsWith("C2S_") && !file.Name.StartsWith("S2C_"))
                            continue;
                        string msgName = file.Name.Substring(0, file.Name.Length - 3);
                        if (genSet.Contains(msgName))
                            continue;
                        genSet.Add(msgName);
                        string line = "";
                        if (file.Name.StartsWith("C2S_"))
                            line = string.Format(
                                "            wsNetMgr.RegisterC2SProtoType(WSNetMsg.{0}, typeof({0}));\r\n", msgName);
                        else
                            line = string.Format(
                                "            wsNetMgr.RegisterS2CProtoType(WSNetMsg.{0}, typeof({0}));\r\n", msgName);
                        lines += line;
                    }
                    content += lines;
                }
            ...
            content += "        }\r\n    }\r\n}";
            string genPath = Application.dataPath + "/_App/AutoCreator/AutoRegister/WSNetMgr/WSNetMgrRegister_AutoCreator.cs";
            WriteFile(genPath, content);
            Debug.Log("[ProjectAutoRegisterTool]处理WSNetMgrRegister完成:" + genPath);
        }
        #endregion
